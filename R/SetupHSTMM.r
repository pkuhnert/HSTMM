...
SetupHSTMM <- function(nT = 10, niter = 100, coords, m = nrow(coords),
                       tune = list(h_lambda = 0.1, h_tau = 0.3, h_K = 20, h_r = 0.1, h_N = 1),
                       ...
                       prior_theta = list(alpha_theta = 1, beta_theta = 1),
  ...
  if(length(prior_lambda$lambda_m) == 1){
    mu_lambda <- rep(prior_lambda$lambda_m, m)
  }
  else if(length(prior_lambda$lambda_m) != m )
      stop("Either specify one prior mean for lambda to reflect all locations or specify
           m separate ones.")
  else{
    mu_lambda <- prior_lambda$lambda_m
  }

  # hyperparameters: tau
  if(length(prior_tau$tau_m) == 1){
    mu_tau <- rep(prior_tau$tau_m, m)
  }
  else  if(length(prior_tau$tau_m) != m )
      stop("Either specify one prior mean for tau to reflect all locations or specify
           m separate ones.")
  else
    mu_tau <- prior_tau$tau_m


  Sigma_lambda <- Sigma_tau <- matrix(NA, nrow = m, ncol = m)
  for(i in 1:m){
    for(k in 1:m){
      Sigma_lambda[i,k] <- prior_lambda$lambda_sig * exp(-2 * abs(dist(matrix(c(longitude[i], latitude[i], longitude[k], latitude[k]),
                                                           byrow = TRUE, ncol = 2))))

      Sigma_tau[i,k] <- prior_tau$tau_sig * exp(-2 * abs(dist(matrix(c(longitude[i], latitude[i], longitude[k], latitude[k]),
                                                               byrow = TRUE, ncol = 2))))
  ...
  list(tune = tune, K = K, r = r, lambda = lambda, tau = tau, N = N,
       ...
       prior_r = prior_r, prior_theta = prior_theta)
